...
static void handle_address(uint64_t address, uint8_t is) {
    ...
    if (strip)
        fprintf(fstrip, "A0x%lx\n", address);
}

static void start_trace(void) {
    if (trace_state == 0) {
        trace_state = 1;
        report("Starting trace");
        if (strip)
            fprintf(fstrip, ">\n");
    }
}
...
void trace_loop(void) {
    ...
    while(data_available()) {
        ...
        if (trace_state == 2 || (trace_state == 0 && header != 0x0)) {
...
void handle_exception(void) {
    ...
    case 0b00000:
        report("PE reset");
        if (strip) {
            fprintf(fstrip, "I:PE reset\n");
        }
        break;
    case 0b00001:
        report("Debug halt");
        if (strip) {
            fprintf(fstrip, "I:Debug halt\n");
        }
        break;
    case 0b00010:
        report("Call");
        if (strip) {
            fprintf(fstrip, "I:Call\n");
        }
        break;
    case 0b00011:
        report("Trap");
        if (strip) {
            fprintf(fstrip, "I:Trap\n");
        }
        break;
    case 0b00100:
        report("System error");
        if (strip) {
            fprintf(fstrip, "I:System error\n");
        }
        break;
    case 0b00110:
        report("Inst debug");
        if (strip) {
            fprintf(fstrip, "I:Inst debug\n");
        }
        break;
    case 0b00111:
        report("Data debug");
        if (strip) {
            fprintf(fstrip, "I:Data debug\n");
        }
        break;
    case 0b01010:
        report("Alignment");
        if (strip) {
            fprintf(fstrip, "I:Alignment\n");
        }
        break;
    case 0b01011:
        report("Inst fault");
        if (strip) {
            fprintf(fstrip, "I:inst fault\n");
        }
        break;
    case 0b01100:
        report("Data fault");
        if (strip) {
            fprintf(fstrip, "I:Data fault\n");
        }
        break;
    case 0b01110:
        report("IRQ");
        if (strip) {
            fprintf(fstrip, "I:IRQ\n");
        }
        break;
    case 0b01111:
        report("FIQ");
        if (strip) {
            fprintf(fstrip, "I:FIQ\n");
        }
        break;
    default:
        report("reserved type");
        if (strip) {
            fprintf(fstrip, "I:reserved type\n");
        }
        break;
    }
}
